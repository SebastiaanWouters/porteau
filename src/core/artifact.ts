  ...
  for (const table of tables) {
    ...
    if (table.scope !== 'DATA') {
      const present = (name: string) =>
        files.some((file) => [name, `${name}.gz`, `${name}.zst`].includes(file))
      if (!present(`${stem}-schema.sql`))
        throw new Error(`Artifact is missing schema for ${table.serialized}`)
      if (table.kind === 'view' && !present(`${stem}-schema-view.sql`))
        throw new Error(`Artifact is missing view definition for ${table.serialized}`)
      if (options.triggers && table.hasTriggers && !present(`${stem}-schema-triggers.sql`))
    ...
    if (table.scope !== 'SCHEMA' && rows !== undefined && rows !== '0') {
      const data = files.some((file) => {
      ...
      if (!data) throw new Error(`Artifact is missing data for ${table.serialized}`)
    }
  }
}
