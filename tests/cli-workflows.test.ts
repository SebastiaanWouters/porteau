...
import { approvedInstall } from '../src/setup/ubuntu.js'
...
describe('setup approval', () => {
  ...
  it.each([
    { extra: [] as string[], json: false },
    { extra: ['--quiet'], json: false },
    { extra: ['--json'], json: true },
  ])('discloses the complete plan before approved mutation: $extra', async ({ extra, json }) => {
    const output: string[] = []
    let receivedApproval: unknown
    const execute = vi.fn(async (_plan: unknown, approval: unknown) => {
      receivedApproval = approval
      const rendered = output.join('\n')
      expect(rendered).toContain('mydumper_1.0.3-1.noble_amd64.deb')
      expect(rendered).toContain('SHA-256')
    })
    expect(
      await executeCli({
        args: ['setup', '--yes', ...extra],
        stdout: (line) => output.push(line),
        stderr: (line) => output.push(line),
        services: {
          collectDiagnostics: async () => needsInstall(),
          executeInstallPlan: execute,
        },
      }),
    ).toBe(0)
    expect(receivedApproval).toBe(approvedInstall)
    if (json) {
      const records = output.map((line) => JSON.parse(line))
      expect(
        records.some((record) => record.type === 'event' && record.event.type === 'plan'),
      ).toBe(true)
    }
  })

  it('never requests approval or mutates when plan disclosure cannot be written', async () => {
    const execute = vi.fn()
  ...
  it('uses interactive confirmation and treats decline or cancellation as cancellation', async () => {
  ...
  it('maps interactive approval to the installer capability token', async () => {
    let receivedApproval: unknown
    const execute = vi.fn(async (_plan: unknown, approval: unknown) => {
      receivedApproval = approval
    })
  ...
  it('never prompts or mutates non-interactively without --yes and skips prompts for no-op plans', async () => {
    const execute = vi.fn()
    const confirm = vi.fn()
    expect(
      await executeCli({
    ...
    expect(
      await executeCli({
        ...
        stdoutTTY: true,
        prompts: { ...noPrompts, confirm },
        services: { collectDiagnostics: async () => diagnostics() },
      }),
    ).toBe(0)
    expect(confirm).not.toHaveBeenCalled()
  })
